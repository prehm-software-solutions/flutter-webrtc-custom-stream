...
- (void)initialize:(NSArray*)networkIgnoreMask
    ...
    if (!_peerConnectionFactory) {
        ...
        int audioDeviceModuleType = 0;
#else
        // Keep the previous iOS behavior. Using CoreAudio ADM on iOS can change audio routing
        // and voice processing/echo cancellation behavior while WebRTC coexists with local playback.
        int audioDeviceModuleType = RTCAudioDeviceModuleTypeAudioEngine;
#endif
...
- (void)handleMethodCall:(FlutterMethodCall*)call result:(FlutterResult)result {
  if ([@"initialize" isEqualToString:call.method]) {
  ...
  } else if ([@"mediaStreamTrackSetNightMode" isEqualToString:call.method]) {
    NSDictionary* argsMap = call.arguments;
    NSString* trackId = argsMap[@"trackId"];
    ...
    if (track != nil && [track isKindOfClass:[LocalVideoTrack class]]) {
      RTCVideoTrack* videoTrack = ((LocalVideoTrack*)track).videoTrack;
      [self mediaStreamTrackSetNightMode:videoTrack enabled:enabled result:result];
    } else {
      if (track == nil) {
        result([FlutterError errorWithCode:@"Track is nil" message:nil details:nil]);
      } else {
